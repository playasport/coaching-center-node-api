...
exports.startMediaCleanupJob = exports.executeMediaCleanupJob = void 0;
 ...
 */
const executeMediaCleanupJob = async () => {
    try {
        logger_1.logger.info('Starting media cleanup job - deleting media soft deleted for 6+ months');
        ...
        for (const center of coachingCenters) {
            try {
                ...
                const coachingCenter = await coachingCenter_model_1.CoachingCenterModel.findById(center._id);
                if (!coachingCenter)
                    continue;
                ...
                if (coachingCenter.documents && Array.isArray(coachingCenter.documents)) {
                    ...
                    for (let i = documentsToDelete.length - 1; i >= 0; i--) {
                        ...
                        try {
                            ...
                            hasChanges = true;
                            totalDeleted++;
                            logger_1.logger.info('Permanently deleted document from S3 and database', {
                            ...
                            logger_1.logger.error('Failed to delete document from S3', {
                                coachingCenterId: center._id.toString(),
                                uniqueId: doc.unique_id,
                                url: doc.url,
                                error,
                        ...
                        if (sportDetail.images && Array.isArray(sportDetail.images)) {
                            ...
                            for (let i = imagesToDelete.length - 1; i >= 0; i--) {
                                ...
                                try {
                                    ...
                                    hasChanges = true;
                                    totalDeleted++;
                                    logger_1.logger.info('Permanently deleted image from S3 and database', {
                                        coachingCenterId: center._id.toString(),
                                    ...
                                    logger_1.logger.error('Failed to delete image from S3', {
                                        coachingCenterId: center._id.toString(),
                                        sportId: sportDetail.sport_id.toString(),
                                        uniqueId: img.unique_id,
                                        url: img.url,
                        ...
                        if (sportDetail.videos && Array.isArray(sportDetail.videos)) {
                            ...
                            for (let i = videosToDelete.length - 1; i >= 0; i--) {
                                ...
                                try {
                                    ...
                                    hasChanges = true;
                                    totalDeleted++;
                                    logger_1.logger.info('Permanently deleted video (and thumbnail) from S3 and database', {
                                        coachingCenterId: center._id.toString(),
                                        sportId: sportDetail.sport_id.toString(),
                                    ...
                                    logger_1.logger.error('Failed to delete video from S3', {
                                        coachingCenterId: center._id.toString(),
                                        sportId: sportDetail.sport_id.toString(),
                                        uniqueId: video.unique_id,
                                        url: video.url,
                ...
                if (hasChanges) {
                    ...
                    logger_1.logger.info('Coaching center updated after media cleanup', {
                        coachingCenterId: center._id.toString(),
                    });
                }
            }
            catch (error) {
                logger_1.logger.error('Failed to process coaching center for media cleanup', {
                    coachingCenterId: center._id.toString(),
                    error,
                });
                totalErrors++;
        ...
        logger_1.logger.info('Media cleanup job completed (coaching center media)', {
        ...
        try {
            logger_1.logger.info('Starting temp folder cleanup - deleting orphaned temp files older than 7 days');
            const sevenDaysAgo = new Date();
            sevenDaysAgo.setDate(sevenDaysAgo.getDate() - 7);
            const client = (0, s3_service_1.getS3Client)();
            if (!client) {
                logger_1.logger.warn('S3 client not configured. Skipping temp file cleanup.');
            }
            else {
                ...
                logger_1.logger.info('Found temp files to delete', { count: filesToDelete.length });
                ...
                for (let i = 0; i < filesToDelete.length; i += batchSize) {
                    ...
                    try {
                        ...
                        logger_1.logger.info('Deleted batch of temp files', {
                        ...
                        logger_1.logger.error('Failed to delete batch of temp files', {
                            batchNumber: Math.floor(i / batchSize) + 1,
                            error,
                        });
                        tempFilesErrors++;
                ...
                logger_1.logger.info('Temp folder cleanup completed', {
                    tempFilesDeleted,
                    tempFilesErrors,
                });
            }
            ...
            logger_1.logger.error('Temp folder cleanup failed', { error });
            tempFilesErrors++;
        }
        logger_1.logger.info('Media cleanup job completed (all)', {
            coachingCentersProcessed: coachingCenters.length,
...
exports.executeMediaCleanupJob = executeMediaCleanupJob;
/**
...
const startMediaCleanupJob = () => {
    ...
    node_cron_1.default.schedule('0 2 * * *', async () => {
        await (0, exports.executeMediaCleanupJob)();
    });
    logger_1.logger.info('Media cleanup cron job scheduled - runs daily at 2 AM');
};
exports.startMediaCleanupJob = startMediaCleanupJob;
//# sourceMappingURL=mediaCleanup.job.js.map