...
exports.getNearbyAcademiesWithRoadDistance = void 0;
 ...
 */
const getNearbyAcademiesWithRoadDistance = async (userLocation, options = {}) => {
    const { maxRadiusKm = 50, limit = GEO_NEAR_LIMIT, extraQuery = {}, } = options;
    try {
        const baseQuery = {
            status: 'published',
            is_active: true,
        ...
        const results = await coachingCenter_model_1.CoachingCenterModel.aggregate(pipeline).exec();
        if (results.length === 0) {
            return [];
        }
        ...
        const roadDistances = await (0, distance_1.calculateDistances)(userLocation.latitude, userLocation.longitude, destinations);
        const combined = results
            ...
            .filter((r) => r.roadDistanceKm <= maxRadiusKm)
            .sort((a, b) => a.roadDistanceKm - b.roadDistanceKm);
        return combined;
    }
    catch (err) {
        logger_1.logger.warn('geoNear academies failed, may need migrate:coaching-center-geo', {
            error: err instanceof Error ? err.message : err,
        });
        return [];
    }
};
exports.getNearbyAcademiesWithRoadDistance = getNearbyAcademiesWithRoadDistance;
//# sourceMappingURL=geoNearAcademies.service.js.map