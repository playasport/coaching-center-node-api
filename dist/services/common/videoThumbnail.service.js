    ...
    try {
        ...
        try {
            ...
            if ((error.message?.includes('NoSuchKey') || error.name === 'NoSuchKey') && videoKey.startsWith('temp/')) {
                ...
                logger_1.logger.info('Video not found in temp location, trying permanent location', {
                    tempKey: videoKey,
                    permanentKey,
                });
                try {
                    ...
                    logger_1.logger.debug('Downloading video from permanent S3 location', { bucket: env_1.config.aws.s3Bucket, key: permanentKey });
                    videoObject = await client.send(getObjectCommand);
                    actualVideoKey = permanentKey;
                    logger_1.logger.info('Video found in permanent location', { permanentKey });
                }
                catch (permanentError) {
                    // If permanent location also fails, throw original error
                    logger_1.logger.error('Video not found in either temp or permanent location', {
                        tempKey: videoKey,
                        permanentKey,
        ...
        logger_1.logger.info('Video downloaded from S3', { videoKey: actualVideoKey, size: videoBuffer.length });
        // Generate thumbnail from buffer
        const thumbnailBuffer = await (0, exports.generateThumbnailFromBuffer)(videoBuffer, actualVideoKey);
