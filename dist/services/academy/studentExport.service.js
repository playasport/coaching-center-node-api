...
const booking_service_1 = require("../client/booking.service");
/**
 ...
 */
const getStudentsForExport = async (academyUserId, filters = {}) => {
    try {
        ...
        logger_1.logger.error('Failed to get students for export:', error);
        throw error;
    }
};
/**
 ...
 */
const transformToExportData = (students) => {
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            for (let i = 0; i < totalPages; i++) {
                try {
                    doc.switchToPage(i);
                    doc.fontSize(8).font('Helvetica').text(`Page ${i + 1} of ${totalPages}`, doc.page.width / 2 - 50, doc.page.height - 40, { align: 'center' });
                }
                catch (error) {
                    // If switchToPage fails, skip page number for that page
                    // This can happen if the document structure isn't fully ready
                    logger_1.logger.warn(`Failed to add page number to page ${i + 1}:`, error);
                }
            }
            doc.end();
        }
...
const exportToCSV = async (academyUserId, filters = {}) => {
    try {
        const students = await getStudentsForExport(academyUserId, filters);
        const exportData = transformToExportData(students);
        const tempFilePath = (0, path_1.join)((0, os_1.tmpdir)(), `enrolled-students-${Date.now()}.csv`);
...
//# sourceMappingURL=studentExport.service.js.map