...
exports.exportToPDF = exports.exportToCSV = exports.exportToExcel = void 0;
 ...
 */
const getBookingsForExport = async (userId, filters = {}) => {
    try {
        ...
        if (filters.type) {
            switch (filters.type) {
                case 'confirmed':
                    query.status = booking_model_1.BookingStatus.CONFIRMED;
                    query['payment.status'] = booking_model_1.PaymentStatus.SUCCESS;
                    break;
                case 'pending':
                    query.status = { $in: [booking_model_1.BookingStatus.SLOT_BOOKED, booking_model_1.BookingStatus.APPROVED, booking_model_1.BookingStatus.REQUESTED, booking_model_1.BookingStatus.PAYMENT_PENDING] };
                    break;
                case 'cancelled':
                    query.status = booking_model_1.BookingStatus.CANCELLED;
                    break;
                case 'rejected':
                    query.status = booking_model_1.BookingStatus.REJECTED;
                    break;
                case 'all':
                default:
        ...
        logger_1.logger.error('Failed to get bookings for export:', error);
        throw error;
    }
};
/**
 ...
 */
const transformToExportData = (bookings) => {
    return bookings.map((booking) => {
        ...
        const paymentStatus = booking.payment?.status === booking_model_1.PaymentStatus.SUCCESS
            ? 'paid'
...
const exportToExcel = async (userId, filters = {}) => {
    try {
        ...
        logger_1.logger.error('Failed to export bookings to Excel:', error);
        throw error;
    }
};
...
const exportToCSV = async (userId, filters = {}) => {
    try {
        const bookings = await getBookingsForExport(userId, filters);
        const exportData = transformToExportData(bookings);
        const tempFilePath = (0, path_1.join)((0, os_1.tmpdir)(), `academy-bookings-${Date.now()}.csv`);
        ...
        const csvContent = await fs_1.promises.readFile(tempFilePath, 'utf-8');
        // Clean up temp file
        await fs_1.promises.unlink(tempFilePath).catch(() => { });
        return csvContent;
    }
    catch (error) {
        logger_1.logger.error('Failed to export bookings to CSV:', error);
        throw error;
    }
};
...
const exportToPDF = async (userId, filters = {}) => {
    try {
        ...
        logger_1.logger.error('Failed to export bookings to PDF:', error);
        throw error;
    }
};
exports.exportToPDF = exportToPDF;
//# sourceMappingURL=bookingExport.service.js.map