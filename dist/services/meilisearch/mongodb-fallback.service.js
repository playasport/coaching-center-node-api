...
exports.mongodbFallback = void 0;
...
class MongodbFallbackService {
     ...
     */
    async searchCoachingCenters(query, options = {}) {
        const { size = 10, from = 0, latitude: userLatitude, longitude: userLongitude, city: filterCity, state: filterState, sportId: filterSportId, sportIds: filterSportIds, gender: filterGender, forDisabled: filterForDisabled, minAge: filterMinAge, maxAge: filterMaxAge, minRating: filterMinRating, radius: radiusKm, sortByDistance = true, } = options;
        try {
            ...
            const matchingSports = await sport_model_1.SportModel.find({
                name: searchRegex,
                is_active: true,
            })
                .select('_id')
            ...
            if (matchingSports.length > 0) {
                const sportIds = matchingSports.map((s) => s._id);
            ...
            let centers = await coachingCenter_model_1.CoachingCenterModel.find(mongoQuery)
                .populate('sports', 'name custom_id')
                .populate('facility', 'name')
                .lean();
            // Transform to match Meilisearch format
            const transformed = await Promise.all(centers.map(async (center) => {
                ...
                if (Array.isArray(sports) && queryLower) {
                    sports.forEach((sport) => {
                        ...
                        if (name && name.toLowerCase().includes(queryLower)) {
                            const id = sport._id?.toString?.() || (sport instanceof mongoose_1.Types.ObjectId && sport.toString());
                            if (id)
                                matchedSportIds.add(id);
                        }
                    });
                }
                ...
                const sortedSports = Array.isArray(sports)
                    ? [...sports].sort((a, b) => {
                        const aId = (a._id?.toString?.() || (a instanceof mongoose_1.Types.ObjectId && a.toString()) || '');
                        const bId = (b._id?.toString?.() || (b instanceof mongoose_1.Types.ObjectId && b.toString()) || '');
                ...
                sortedSports.forEach((sport) => {
                    ...
                    else if (sport instanceof mongoose_1.Types.ObjectId) {
                ...
                if (latitude && longitude && userLatitude && userLongitude) {
                    distance = await (0, distance_1.calculateDistance)(userLatitude, userLongitude, latitude, longitude);
                }
                // Check if query matches sports
                const isSportMatch = sportsNames.some((sport) => sport.toLowerCase().includes(queryLower));
                return {
            ...
            if (radiusKm != null &&
                ...
                userLatitude != null &&
                userLongitude != null) {
                filtered = transformed.filter((c) => c.distance != null && c.distance <= radiusKm);
            }
            ...
            logger_1.logger.error('MongoDB fallback search coaching centers error:', error);
            return {
                hits: [],
    ...
    async searchSports(query, options = {}) {
        ...
        try {
            ...
            const sports = await sport_model_1.SportModel.find(mongoQuery)
                .sort({ name: 1 })
                .limit(size + from)
                .lean();
            const transformed = sports.slice(from).map((sport) => ({
            ...
            logger_1.logger.error('MongoDB fallback search sports error:', error);
            return {
                hits: [],
                estimatedTotalHits: 0,
                processingTimeMs: 0,
     ...
     */
    async searchReels(query, options = {}) {
        const { size = 10, from = 0 } = options;
        try {
            const searchRegex = new RegExp(query, 'i');
            const mongoQuery = {
            ...
            const reels = await reel_model_1.ReelModel.find(mongoQuery)
                .sort({ viewsCount: -1, createdAt: -1 })
                .limit(size + from)
                .lean();
            const transformed = reels.slice(from).map((reel) => ({
            ...
            logger_1.logger.error('MongoDB fallback search reels error:', error);
            return {
                hits: [],
                estimatedTotalHits: 0,
                processingTimeMs: 0,
     ...
     */
    async searchStreamHighlights(query, options = {}) {
        const { size = 10, from = 0 } = options;
        try {
            const searchRegex = new RegExp(query, 'i');
            const mongoQuery = {
            ...
            const highlights = await streamHighlight_model_1.StreamHighlightModel.find(mongoQuery)
                .sort({ viewsCount: -1, createdAt: -1 })
                .limit(size + from)
                .lean();
            const transformed = highlights.slice(from).map((highlight) => ({
            ...
            logger_1.logger.error('MongoDB fallback search stream highlights error:', error);
            return {
                hits: [],
                estimatedTotalHits: 0,
                processingTimeMs: 0,
...
exports.mongodbFallback = new MongodbFallbackService();
//# sourceMappingURL=mongodb-fallback.service.js.map