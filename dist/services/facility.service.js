...
const getAllFacilities = async () => {
    try {
        const facilities = await facility_model_1.FacilityModel.find({ is_active: true })
...
const findOrCreateFacility = async (facility) => {
    try {
        ...
        if (typeof facility === 'string') {
            ...
            const existingFacility = await facility_model_1.FacilityModel.findById(facilityId);
            if (!existingFacility) {
