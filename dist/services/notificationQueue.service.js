...
const isChannelEnabled = (channel) => {
    if (!env_1.config.notification.enabled) {
        return false;
    }
    switch (channel) {
        case 'sms':
        ...
        case 'email':
            return env_1.config.email.enabled;
        case 'whatsapp':
            return env_1.config.notification.whatsapp.enabled && env_1.config.sms.enabled && !!env_1.config.twilio.accountSid; // WhatsApp uses Twilio
        case 'push':
            return env_1.config.notification.push.enabled; // Check push notification config
        default:
            return false;
    }
};
