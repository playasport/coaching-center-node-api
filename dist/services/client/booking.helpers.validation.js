...
exports.validateParticipantEligibility = exports.validateSlotAvailability = exports.validateParticipantEnrollment = exports.validateBatchAndCenter = exports.validateAndFetchParticipants = void 0;
...
exports.validateAndFetchParticipants = validateAndFetchParticipants;
/**
...
const validateBatchAndCenter = async (batchId) => {
    ...
    if (batch.is_deleted) {
        throw new ApiError_1.ApiError(400, 'Batch has been deleted and is not available for booking');
    }
    // Validate batch is active (not disabled)
    if (!batch.is_active) {
        throw new ApiError_1.ApiError(400, 'Batch is disabled and not available for booking');
    }
    // Check if batch is published
    if (batch.status !== batchStatus_enum_1.BatchStatus.PUBLISHED) {
        throw new ApiError_1.ApiError(400, 'Batch is not published and not available for booking');
    }
    // Use populated coaching center data (already fetched in populate)
    const populatedCenter = batch.center;
    if (!populatedCenter) {
    ...
    const coachingCenter = await coachingCenter_model_1.CoachingCenterModel.findById(centerId)
    ...
    if (coachingCenter.is_deleted) {
        throw new ApiError_1.ApiError(400, 'Coaching center has been deleted and is not available for booking');
    }
    // Validate coaching center is active (not disabled)
    if (!coachingCenter.is_active) {
        throw new ApiError_1.ApiError(400, 'Coaching center is disabled and not available for booking');
    }
    // Validate coaching center is published
    if (coachingCenter.status !== coachingCenterStatus_enum_1.CoachingCenterStatus.PUBLISHED) {
        throw new ApiError_1.ApiError(400, 'Coaching center is not published and not available for booking');
    }
    // Validate coaching center is approved
    if (coachingCenter.approval_status !== 'approved') {
        throw new ApiError_1.ApiError(400, 'Coaching center is not approved and not available for booking');
    }
    return { batch, coachingCenter };
};
exports.validateBatchAndCenter = validateBatchAndCenter;
/**
...
const validateParticipantEnrollment = async (participantIds, batchId) => {
    ...
    const existingBookings = await booking_model_1.BookingModel.find({
        ...
        status: { $in: [booking_model_1.BookingStatus.SLOT_BOOKED, booking_model_1.BookingStatus.APPROVED, booking_model_1.BookingStatus.PAYMENT_PENDING, booking_model_1.BookingStatus.CONFIRMED, booking_model_1.BookingStatus.REQUESTED, booking_model_1.BookingStatus.PENDING] }, // Include legacy statuses for backward compatibility
        is_deleted: false,
    })
        .select('participants')
        .lean();
    if (existingBookings.length > 0) {
        ...
        if (enrolledParticipantIds.size > 0) {
            ...
            const enrolledIdsArray = Array.from(enrolledParticipantIds).map(id => new mongoose_1.Types.ObjectId(id));
            const enrolledParticipants = await participant_model_1.ParticipantModel.find({
...
exports.validateParticipantEnrollment = validateParticipantEnrollment;
/**
...
exports.validateSlotAvailability = validateSlotAvailability;
/**
...
const validateParticipantEligibility = async (participants, batch, coachingCenter) => {
    ...
    for (const participant of participants) {
        ...
        if (participant.gender !== null && participant.gender !== undefined && participant.gender !== '') {
            ...
            if (participantGender) {
                ...
                if (batch.gender && Array.isArray(batch.gender) && batch.gender.length > 0) {
                    const batchAllowedGenders = batch.gender.map((g) => g.toLowerCase());
                    if (!batchAllowedGenders.includes(participantGender.toLowerCase())) {
                        const allowedGendersStr = batch.gender.join(', ');
                        throw new ApiError_1.ApiError(400, `Participant ${participant.firstName || participant._id} gender (${participantGender}) is not allowed for this batch. Batch allowed genders: ${allowedGendersStr}`);
                    }
                }
                ...
                if (coachingCenter.allowed_genders && Array.isArray(coachingCenter.allowed_genders) && coachingCenter.allowed_genders.length > 0) {
                    if (!coachingCenter.allowed_genders.includes(participantGender)) {
                        ...
                        throw new ApiError_1.ApiError(400, `Participant ${participant.firstName || participant._id} gender (${participantGender}) is not allowed by the coaching center. Allowed genders: ${allowedGendersStr}`);
                    }
                }
        ...
        if (!batch.is_allowed_disabled && hasDisability) {
            throw new ApiError_1.ApiError(400, `Participant ${participant.firstName || participant._id || 'Unknown'} has a disability. This batch (${batch.name || 'Unknown'}) does not allow disabled participants.`);
        }
        ...
        if (coachingCenter.is_only_for_disabled) {
            ...
            if (!hasDisability) {
                throw new ApiError_1.ApiError(400, `Participant ${participant.firstName || participant._id || 'Unknown'} does not have a disability. This coaching center (${coachingCenter.center_name || 'Unknown'}) is exclusively for disabled participants.`);
            }
        }
        else {
            ...
            if (!coachingCenter.allowed_disabled && hasDisability) {
                throw new ApiError_1.ApiError(400, `Participant ${participant.firstName || participant._id || 'Unknown'} has a disability. This coaching center (${coachingCenter.center_name || 'Unknown'}) does not allow disabled participants.`);
            }
        }
...
exports.validateParticipantEligibility = validateParticipantEligibility;
//# sourceMappingURL=booking.helpers.validation.js.map