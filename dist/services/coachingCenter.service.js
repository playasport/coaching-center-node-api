...
exports.removeMediaFromCoachingCenter = exports.deleteCoachingCenter = exports.toggleCoachingCenterStatus = exports.updateCoachingCenter = exports.getCoachingCentersByUser = exports.getCoachingCenterById = exports.createCoachingCenter = void 0;
...
const createCoachingCenter = async (data, userId) => {
    ...
    const sportIds = data.sports.map((id) => new mongoose_1.Types.ObjectId(id));
    ...
    if (data.facility && Array.isArray(data.facility) && data.facility.length > 0) {
        ...
        const facilityPromises = data.facility.map(async (facilityInput) => {
            if (typeof facilityInput === 'string') {
                ...
                if (!mongoose_1.Types.ObjectId.isValid(facilityInput)) {
                    throw new ApiError_1.ApiError(400, (0, i18n_1.t)('coachingCenter.facility.invalidId', { id: facilityInput }));
                }
                const facilityId = new mongoose_1.Types.ObjectId(facilityInput);
                const facilityExists = await facility_model_1.FacilityModel.findById(facilityId);
                if (!facilityExists) {
                    throw new ApiError_1.ApiError(400, (0, i18n_1.t)('coachingCenter.facility.notFound', { id: facilityInput }));
                }
                return facilityId;
            }
            else {
                // New facility object - create it
                const facilityId = await (0, facility_service_1.findOrCreateFacility)(facilityInput);
                if (!facilityId) {
        ...
        facilityIds = resolvedFacilities.filter((id) => id !== null);
    }
    ...
    if (data.sport_details && Array.isArray(data.sport_details) && data.sport_details.length > 0) {
        sportDetails = data.sport_details.map((detail) => ({
            ...detail,
            sport_id: new mongoose_1.Types.ObjectId(detail.sport_id),
        }));
    }
    // ============================================
    ...
    try {
        ...
        coachingCenter = new coachingCenter_model_1.CoachingCenterModel(coachingCenterData);
        await coachingCenter.save();
        logger_1.logger.info(`Coaching center saved successfully: ${coachingCenter._id} (${coachingCenter.center_name})`);
    }
    catch (saveError) {
        logger_1.logger.error('Failed to save coaching center after validations passed', {
    ...
    if (data.status === 'published' && coachingCenter && coachingCenter._id) {
        try {
            logger_1.logger.info('Moving media files to permanent location for published coaching center', {
            ...
            const freshCenter = await coachingCenter_model_1.CoachingCenterModel.findById(coachingCenter._id).lean();
            if (!freshCenter) {
                throw new ApiError_1.ApiError(404, 'Coaching center not found after save');
            }
            // Convert to CoachingCenter type with proper _id
            const centerForMove = {
                ...freshCenter,
                id: freshCenter.id || freshCenter._id?.toString() || '',
            };
            ...
            const updatedCenter = await coachingCenter_model_1.CoachingCenterModel.findById(coachingCenter._id)
                ...
                .lean();
            if (updatedCenter) {
                logger_1.logger.info(`Coaching center created with permanent URLs: ${coachingCenter._id} (${coachingCenter.center_name})`);
                // Verify URLs are permanent
                if (updatedCenter.logo && updatedCenter.logo.includes('temp/')) {
                    logger_1.logger.warn('Logo URL still contains temp/ after move', { logo: updatedCenter.logo });
                }
                ...
                if (updatedCenter.sport_details && Array.isArray(updatedCenter.sport_details) && updatedCenter.sport_details.length > 0) {
                    ...
                    if (firstSportDetail.images?.[0]?.url?.includes('temp/')) {
                        logger_1.logger.warn('Image URLs still contain temp/ after move', {
                ...
                try {
                    ...
                    logger_1.logger.error('Failed to enqueue thumbnail generation, but coaching center was created', {
                        centerId: coachingCenter._id,
                ...
                logger_1.logger.warn('Could not refetch coaching center after media move, returning original', {
                    centerId: coachingCenter._id,
                });
            }
        }
        catch (postProcessError) {
            logger_1.logger.error('Error during file moving for published center', {
                centerId: coachingCenter._id,
    ...
    try {
        const populatedCenter = await coachingCenter_model_1.CoachingCenterModel.findById(coachingCenter._id)
            .populate('sports', 'custom_id name logo is_popular')
            .populate('facility', 'custom_id name description icon')
            .populate({
            path: 'user',
        ...
        logger_1.logger.warn('Failed to populate coaching center, returning basic object', {
            centerId: coachingCenter._id,
...
const getCoachingCenterById = async (id) => {
    try {
        const coachingCenter = await coachingCenter_model_1.CoachingCenterModel.findById(id)
            .populate('sports', 'custom_id name logo is_popular')
...
const getCoachingCentersByUser = async (userId, page = 1, limit = env_1.config.pagination.defaultLimit) => {
    try {
        ...
        logger_1.logger.info('Coaching centers fetched by user', {
            userId,
            page: pageNumber,
            limit: pageSize,
            total,
        ...
        logger_1.logger.error('Failed to fetch coaching centers by user:', error);
...
const updateCoachingCenter = async (id, data) => {
    try {
        ...
        if (data.sports !== undefined) {
            ...
            const sportIds = data.sports.map((sportId) => new mongoose_1.Types.ObjectId(sportId));
            const sportsCount = await sport_model_1.SportModel.countDocuments({ _id: { $in: sportIds } });
            if (sportsCount !== data.sports.length) {
                throw new ApiError_1.ApiError(400, (0, i18n_1.t)('coachingCenter.sports.invalid'));
            }
        ...
        if (data.facility !== undefined) {
            if (data.facility && Array.isArray(data.facility) && data.facility.length > 0) {
                ...
                const resolvedFacilities = await Promise.all(facilityPromises);
                // Filter out any null values (shouldn't happen due to error throwing, but TypeScript needs this)
                updates.facility = resolvedFacilities.filter((id) => id !== null);
            }
            else {
                updates.facility = [];
            }
        ...
        if (data.sport_details !== undefined) {
            if (data.sport_details && Array.isArray(data.sport_details) && data.sport_details.length > 0) {
                ...
                for (const newDetail of data.sport_details) {
                    const sportId = new mongoose_1.Types.ObjectId(newDetail.sport_id);
                    const existingIndex = updatedSportDetails.findIndex((detail) => detail.sport_id.toString() === sportId.toString());
                    if (existingIndex >= 0) {
                        ...
                        if (newDetail.images && Array.isArray(newDetail.images)) {
                            ...
                            for (const newImage of newDetail.images) {
                                const imageIndex = newImages.findIndex((img) => img.unique_id === newImage.unique_id);
                                if (imageIndex >= 0) {
                                    ...
                                    newImages.push({
                                        unique_id: newImage.unique_id || (0, uuid_1.v4)(),
                                        url: newImage.url,
                        ...
                        if (newDetail.videos && Array.isArray(newDetail.videos)) {
                            ...
                            for (const newVideo of newDetail.videos) {
                                const videoIndex = newVideos.findIndex((video) => video.unique_id === newVideo.unique_id);
                                if (videoIndex >= 0) {
                                    ...
                                    newVideos.push({
                                        unique_id: newVideo.unique_id || (0, uuid_1.v4)(),
                                        url: newVideo.url,
                        ...
                        updatedSportDetails.push({
                            ...
                            images: (newDetail.images || []).map((img) => ({
                                unique_id: img.unique_id || (0, uuid_1.v4)(),
                                url: img.url,
                                is_active: img.is_active !== undefined ? img.is_active : true,
                                is_deleted: false,
                                deletedAt: null,
                            })),
                            videos: (newDetail.videos || []).map((video) => ({
                                unique_id: video.unique_id || (0, uuid_1.v4)(),
                                url: video.url,
        ...
        if (data.center_name !== undefined)
            updates.center_name = data.center_name;
        if (data.rules_regulation !== undefined)
            updates.rules_regulation = data.rules_regulation;
        ...
        if (data.operational_timing !== undefined)
            updates.operational_timing = data.operational_timing;
        if (data.documents !== undefined)
            updates.documents = data.documents;
        if (data.bank_information !== undefined)
            updates.bank_information = data.bank_information;
        if (data.allowed_genders !== undefined)
            updates.allowed_genders = data.allowed_genders;
        if (data.allowed_disabled !== undefined)
            updates.allowed_disabled = data.allowed_disabled;
        if (data.is_only_for_disabled !== undefined)
            updates.is_only_for_disabled = data.is_only_for_disabled;
        ...
        if (data.status === 'published' && previousStatus !== 'published') {
            ...
            const finalData = {
                ...
                sports: data.sports !== undefined ? data.sports : existingCenter.sports?.map((s) => s.toString()),
            ...
            if (Array.isArray(finalData.sport_details)) {
                finalData.sport_details.forEach((sportDetail, index) => {
                    if (!sportDetail.sport_id || (typeof sportDetail.sport_id === 'string' && sportDetail.sport_id.trim().length === 0)) {
                        throw new ApiError_1.ApiError(400, `${(0, i18n_1.t)('validation.coachingCenter.sports.required')} (sport_details[${index}].sport_id)`);
                    }
                    if (!sportDetail.description || sportDetail.description.trim().length === 0) {
                        throw new ApiError_1.ApiError(400, `${(0, i18n_1.t)('coachingCenter.description.required')} (sport_details[${index}].description)`);
                    }
                    if (sportDetail.description.length < 5) {
                        throw new ApiError_1.ApiError(400, `${(0, i18n_1.t)('coachingCenter.description.minLength')} (sport_details[${index}].description)`);
                    }
                });
            }
            ...
            if (!finalData.location.address.line2 || !finalData.location.address.city || !finalData.location.address.state || !finalData.location.address.pincode) {
                throw new ApiError_1.ApiError(400, (0, i18n_1.t)('address.line2Required'));
            }
            if (!finalData.operational_timing) {
                throw new ApiError_1.ApiError(400, (0, i18n_1.t)('coachingCenter.operationalTiming.openingTimeRequired'));
            }
            if (!finalData.bank_information) {
                throw new ApiError_1.ApiError(400, (0, i18n_1.t)('coachingCenter.bankInformation.bankNameRequired'));
            }
        }
        ...
        const updatedCenter = await coachingCenter_model_1.CoachingCenterModel.findByIdAndUpdate(id, { $set: updates }, { new: true, runValidators: true })
            .populate('sports', 'custom_id name logo is_popular')
            .populate('facility', 'custom_id name description icon')
            .populate({
            path: 'user',
        ...
        if (previousStatus === 'draft' && (data.status === 'published' || updatedCenter.status === 'published')) {
            ...
            const refreshedCenter = await coachingCenter_model_1.CoachingCenterModel.findById(id)
                .populate('sports', 'custom_id name logo is_popular')
                .populate('facility', 'custom_id name description icon')
                .populate({
                path: 'user',
                ...
                .lean();
            if (refreshedCenter) {
                logger_1.logger.info(`Coaching center updated with permanent URLs: ${id} (${updatedCenter.center_name})`);
                // Verify URLs are permanent
                if (refreshedCenter.logo && refreshedCenter.logo.includes('temp/')) {
                    logger_1.logger.warn('Logo URL still contains temp/ after move', { logo: refreshedCenter.logo });
                }
                // Check sport_details for temp URLs
        ...
        logger_1.logger.info(`Coaching center updated: ${id} (${updatedCenter.center_name})`);
        return updatedCenter;
    }
    catch (error) {
        ...
        logger_1.logger.error('Failed to update coaching center:', error);
        throw new ApiError_1.ApiError(500, (0, i18n_1.t)('coachingCenter.update.failed'));
    }
};
...
const enqueueThumbnailGenerationForVideos = async (coachingCenter) => {
    try {
        const coachingCenterId = coachingCenter._id?.toString();
        if (!coachingCenterId) {
            logger_1.logger.warn('Cannot enqueue thumbnail generation: coaching center ID not found');
            return;
        }
        if (!coachingCenter.sport_details || !Array.isArray(coachingCenter.sport_details)) {
            logger_1.logger.debug('No sport_details found, skipping thumbnail generation');
            return;
        }
        let videoCount = 0;
            ...
            if (sportDetail.videos && Array.isArray(sportDetail.videos)) {
                for (let videoIndex = 0; videoIndex < sportDetail.videos.length; videoIndex++) {
                    ...
                    if (video.url && !video.is_deleted && !video.thumbnail) {
                        ...
                        videoCount++;
                        logger_1.logger.debug('Enqueued thumbnail generation for video', {
                            coachingCenterId,
                            videoUrl: video.url,
                            sportIndex,
                            videoIndex,
        ...
        if (videoCount > 0) {
            logger_1.logger.info('Enqueued thumbnail generation for videos', {
                coachingCenterId,
                videoCount,
            });
        }
        else {
            logger_1.logger.debug('No videos found for thumbnail generation', { coachingCenterId });
        }
    }
    catch (error) {
        logger_1.logger.error('Failed to enqueue thumbnail generation for videos', {
...
const moveMediaFilesToPermanent = async (coachingCenter) => {
    try {
        ...
        const coachingCenterId = coachingCenter._id || coachingCenter.id || null;
        if (!coachingCenterId) {
            logger_1.logger.error('Coaching center ID not found', { coachingCenter });
        ...
        if (coachingCenter.logo) {
            ...
            logger_1.logger.info('Added logo URL to move list', { url: coachingCenter.logo });
        }
        ...
        if (coachingCenter.sport_details && Array.isArray(coachingCenter.sport_details)) {
            coachingCenter.sport_details.forEach((sportDetail) => {
                ...
                if (sportDetail.images && Array.isArray(sportDetail.images)) {
                    sportDetail.images.forEach((img) => {
                        if (img && img.url && !img.is_deleted) {
                            fileUrls.push(img.url);
                            logger_1.logger.info('Added sport image URL to move list', { url: img.url, sportId: sportDetail.sport_id });
                        }
                    });
                }
                // Collect videos from sport_details
                if (sportDetail.videos && Array.isArray(sportDetail.videos)) {
                    sportDetail.videos.forEach((vid) => {
                        if (vid && vid.url && !vid.is_deleted) {
                            fileUrls.push(vid.url);
                            logger_1.logger.info('Added sport video URL to move list', { url: vid.url, sportId: sportDetail.sport_id });
                        }
                        // Also collect thumbnail if it exists
                        if (vid && vid.thumbnail && !vid.is_deleted) {
                            fileUrls.push(vid.thumbnail);
                            logger_1.logger.info('Added video thumbnail URL to move list', { url: vid.thumbnail, sportId: sportDetail.sport_id });
                        }
                    });
                }
            });
        ...
        if (coachingCenter.documents && Array.isArray(coachingCenter.documents)) {
            coachingCenter.documents.forEach((doc) => {
                if (doc && doc.url && !doc.is_deleted) {
                    ...
                    logger_1.logger.info('Added document URL to move list', { url: doc.url });
                }
            });
        }
        if (fileUrls.length === 0) {
            logger_1.logger.warn('No media files found to move to permanent location', { coachingCenterId: coachingCenterIdStr });
            return;
        }
        logger_1.logger.info(`Moving ${fileUrls.length} media files to permanent locations`, {
            coachingCenterId: coachingCenterIdStr,
            fileUrls,
        });
        ...
        fileUrls.forEach((tempUrl, index) => {
            if (permanentUrls[index]) {
                ...
                logger_1.logger.info('URL mapping', {
                    tempUrl,
                ...
                logger_1.logger.warn('No permanent URL found for temp URL', { tempUrl, index });
            }
        });
        logger_1.logger.info(`Successfully moved ${permanentUrls.length} files`, {
            coachingCenterId: coachingCenterIdStr,
        ...
        if (coachingCenter.logo) {
            ...
            if (permanentLogoUrl) {
                logger_1.logger.info('Updating logo URL', {
                ...
                logger_1.logger.warn('Logo URL not found in URL map', {
        ...
        if (coachingCenter.sport_details && Array.isArray(coachingCenter.sport_details)) {
            const sportDetailsUpdates = coachingCenter.sport_details.map((sportDetail) => {
                const updatedDetail = {
                    ...sportDetail,
                    images: [],
                    videos: [],
                ...
                if (Array.isArray(sportDetail.images)) {
                    updatedDetail.images = sportDetail.images.map((img) => {
                        if (img && img.url && !img.is_deleted) {
                            const permanentUrl = urlMap.get(img.url);
                            if (permanentUrl) {
                                logger_1.logger.info('Updating sport image URL', {
                                    oldUrl: img.url,
                                    newUrl: permanentUrl,
                                ...
                                logger_1.logger.warn('Sport image URL not found in URL map', {
                                    imageUrl: img.url,
                                    sportId: sportDetail.sport_id,
                                });
                            }
                ...
                if (Array.isArray(sportDetail.videos)) {
                    updatedDetail.videos = sportDetail.videos.map((vid) => {
                        const updatedVideo = { ...vid };
                        if (vid && vid.url && !vid.is_deleted) {
                            const permanentUrl = urlMap.get(vid.url);
                            if (permanentUrl) {
                                logger_1.logger.info('Updating sport video URL', {
                                    oldUrl: vid.url,
                                    newUrl: permanentUrl,
                                    sportId: sportDetail.sport_id,
                                });
                                ...
                                logger_1.logger.warn('Sport video URL not found in URL map', {
                                    videoUrl: vid.url,
                                    sportId: sportDetail.sport_id,
                                });
                            }
                        ...
                        if (vid && vid.thumbnail && !vid.is_deleted) {
                            ...
                            if (permanentThumbnailUrl) {
                                logger_1.logger.info('Updating video thumbnail URL', {
                                ...
                                logger_1.logger.warn('Video thumbnail URL not found in URL map', {
            ...
            logger_1.logger.info('Prepared sport_details updates', {
        ...
        if (coachingCenter.documents && Array.isArray(coachingCenter.documents)) {
            updates.documents = coachingCenter.documents.map((doc) => {
                if (doc && doc.url && !doc.is_deleted) {
                    ...
                    if (permanentUrl) {
                        logger_1.logger.info('Updating document URL', {
                            oldUrl: doc.url,
                            newUrl: permanentUrl,
                        });
                        ...
                        logger_1.logger.warn('Document URL not found in URL map', {
                            documentUrl: doc.url,
                        });
                    }
                }
            ...
            logger_1.logger.info('Prepared documents updates', {
        ...
        if (Object.keys(updates).length > 0) {
            ...
            if (coachingCenterId) {
                logger_1.logger.info('Updating coaching center with permanent URLs', {
                ...
                logger_1.logger.info('Update query prepared', {
                    coachingCenterId: coachingCenterId.toString(),
                ...
                logger_1.logger.info('URLs mapping', {
                ...
                let centerObjectId;
                if (coachingCenterId instanceof mongoose_1.Types.ObjectId) {
                    centerObjectId = coachingCenterId;
                }
                else if (mongoose_1.Types.ObjectId.isValid(coachingCenterId)) {
                    centerObjectId = new mongoose_1.Types.ObjectId(coachingCenterId);
                }
                else {
                    logger_1.logger.error('Invalid coaching center ID format', {
                        coachingCenterId,
                        coachingCenterIdStr,
                    });
                ...
                logger_1.logger.info('Executing database update', {
                    centerObjectId: centerObjectId.toString(),
                    updateQueryKeys: Object.keys(updateQuery),
                });
                const updateResult = await coachingCenter_model_1.CoachingCenterModel.findByIdAndUpdate(centerObjectId, { $set: updateQuery }, { new: true, runValidators: false } // new: true returns updated document
                );
                if (!updateResult) {
                    logger_1.logger.error('Failed to update coaching center with permanent URLs - updateResult is null', {
                        coachingCenterId: coachingCenterIdStr,
                        centerObjectId: centerObjectId.toString(),
                        updateQuery,
                    });
                ...
                logger_1.logger.info(`Database update completed - checking result`, {
                    coachingCenterId: coachingCenterIdStr,
                ...
                logger_1.logger.info('Verified update - permanent URLs saved', {
                    coachingCenterId: coachingCenterIdStr,
                ...
                const verifyCenter = await coachingCenter_model_1.CoachingCenterModel.findById(centerObjectId).lean();
                if (verifyCenter) {
                    ...
                    if (verifyCenter.sport_details && Array.isArray(verifyCenter.sport_details)) {
                        verifyCenter.sport_details.forEach((sportDetail) => {
                            if (sportDetail.images && Array.isArray(sportDetail.images)) {
                                sportDetail.images.forEach((img) => {
                    ...
                    if (verifyCenter.documents && Array.isArray(verifyCenter.documents)) {
                        verifyCenter.documents.forEach((doc) => {
                    ...
                    if (hasTempUrls) {
                        logger_1.logger.error('CRITICAL: Temp URLs still present after update!', {
                            coachingCenterId: coachingCenterId.toString(),
                        ...
                        logger_1.logger.info('✅ All URLs successfully updated to permanent locations', {
                            coachingCenterId: coachingCenterId.toString(),
                        });
                    }
                }
                ...
                logger_1.logger.error('Coaching center ID not found', {
                    coachingCenter: JSON.stringify(coachingCenter, null, 2),
                });
            }
        }
        else {
            logger_1.logger.warn('No updates to apply - updates object is empty', {
                coachingCenterId: coachingCenter._id?.toString() || 'unknown',
                fileUrls,
                permanentUrls,
                hasLogo: !!coachingCenter.logo,
                hasDocuments: !!coachingCenter.documents && coachingCenter.documents.length > 0,
        ...
        const coachingCenterId = coachingCenter._id?.toString() || 'unknown';
        logger_1.logger.error('Failed to move media files to permanent location', {
            coachingCenterId,
            error: error instanceof Error ? error.message : error,
...
const toggleCoachingCenterStatus = async (id) => {
    try {
        ...
        const updatedCenter = await coachingCenter_model_1.CoachingCenterModel.findByIdAndUpdate(id, { is_active: newActiveStatus }, { new: true, runValidators: true })
            .populate('sports', 'custom_id name logo is_popular')
            .populate('sport_details.sport_id', 'custom_id name logo is_popular')
            .populate('facility', 'custom_id name description icon')
            .populate({
        ...
        logger_1.logger.info('Coaching center active status toggled', {
            id,
        ...
        logger_1.logger.error('Failed to toggle coaching center active status:', error);
...
const deleteCoachingCenter = async (id) => {
    try {
        ...
        await coachingCenter_model_1.CoachingCenterModel.findByIdAndUpdate(id, {
            is_deleted: true,
            deletedAt: new Date(),
        }, { runValidators: true });
        logger_1.logger.info('Coaching center soft deleted successfully', {
            coachingCenterId: id,
        });
    }
    catch (error) {
...
const removeMediaFromCoachingCenter = async (coachingCenterId, mediaType, uniqueId, sportId // Required for image/video (sport-specific media)
    ...
    try {
        ...
        const sportDetailIndex = coachingCenter.sport_details?.findIndex((detail) => detail.sport_id.toString() === sportId && detail);
