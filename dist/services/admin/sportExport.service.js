 ...
 */
const getSportsForExport = async (filters = {}) => {
    try {
        const query = {};
        // Apply filters
        if (filters.search) {
        ...
        const sports = await sport_model_1.SportModel.find(query)
            .sort({ name: 1 })
            .lean();
        return sports;
    }
    catch (error) {
        logger_1.logger.error('Failed to get sports for export:', error);
        throw error;
    }
};
/**
 ...
 */
const transformToExportData = (sports) => {
...
const exportToExcel = async (filters = {}) => {
    try {
        ...
        logger_1.logger.error('Failed to export sports to Excel:', error);
        throw error;
    }
};
...
const exportToPDF = async (filters = {}) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            logger_1.logger.error('Failed to export sports to PDF:', error);
            reject(error);
        }
    });
};
...
const exportToCSV = async (filters = {}) => {
    try {
        const sports = await getSportsForExport(filters);
        const exportData = transformToExportData(sports);
        const tempFilePath = (0, path_1.join)((0, os_1.tmpdir)(), `sports-${Date.now()}.csv`);
        ...
        logger_1.logger.error('Failed to export sports to CSV:', error);
        throw error;
    }
};
exports.exportToCSV = exportToCSV;
//# sourceMappingURL=sportExport.service.js.map