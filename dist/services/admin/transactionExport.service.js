 ...
 */
const getTransactionsForExport = async (filters = {}) => {
    try {
        const query = {};
        ...
        if (filters.bookingId) {
            const queryId = mongoose_1.Types.ObjectId.isValid(filters.bookingId)
                ? { _id: new mongoose_1.Types.ObjectId(filters.bookingId) }
        ...
        const transactions = await transaction_model_1.TransactionModel.find(query)
            ...
            .sort({ createdAt: -1 })
            .lean();
        return transactions.filter((tx) => tx.booking); // Filter out transactions with deleted bookings
    }
    catch (error) {
        logger_1.logger.error('Failed to get transactions for export:', error);
        throw error;
    }
};
/**
 ...
 */
const transformToExportData = (transactions) => {
...
const exportToExcel = async (filters = {}) => {
    try {
        ...
        logger_1.logger.error('Failed to export transactions to Excel:', error);
        throw error;
    }
};
...
const exportToCSV = async (filters = {}) => {
    try {
        const transactions = await getTransactionsForExport(filters);
        const exportData = transformToExportData(transactions);
        const tempFilePath = (0, path_1.join)((0, os_1.tmpdir)(), `transactions-${Date.now()}.csv`);
...
const exportToPDF = async (filters = {}) => {
    try {
        ...
        logger_1.logger.error('Failed to export transactions to PDF:', error);
        throw error;
    }
};
exports.exportToPDF = exportToPDF;
//# sourceMappingURL=transactionExport.service.js.map