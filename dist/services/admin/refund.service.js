...
exports.getRefundDetails = exports.createRefund = void 0;
 ...
 */
const createRefund = async (bookingId, adminUserId, refundData, options) => {
    try {
        // Find booking
        ...
        if (booking.status !== booking_model_1.BookingStatus.CONFIRMED) {
            throw new ApiError_1.ApiError(400, `Cannot refund booking in ${booking.status} status. Only confirmed bookings can be refunded.`);
        }
        // Validate payment status
        if (booking.payment.status !== booking_model_1.PaymentStatus.SUCCESS) {
            throw new ApiError_1.ApiError(400, `Cannot refund booking with payment status: ${booking.payment.status}. Payment must be successful.`);
        }
        if (!booking.payment.razorpay_payment_id) {
            throw new ApiError_1.ApiError(400, 'Razorpay payment ID not found. Cannot process refund.');
        }
        ...
        if (refundAmount > booking.amount) {
            throw new ApiError_1.ApiError(400, 'Refund amount cannot exceed booking amount');
        }
        // Check if refund already exists
        const existingRefundTransaction = await transaction_model_1.TransactionModel.findOne({
            booking: booking._id,
        ...
        if (existingRefundTransaction) {
            throw new ApiError_1.ApiError(400, 'Refund already processed for this booking');
        }
        // Create refund in Razorpay
        const razorpayRefund = await razorpayRoute_service_1.razorpayRouteService.createRefund(booking.payment.razorpay_payment_id, refundData.amount, // undefined = full refund
        {
        ...
        const updatedBooking = await booking_model_1.BookingModel.findOneAndUpdate({ id: bookingId }, {
            $set: {
                status: isFullRefund ? booking_model_1.BookingStatus.CANCELLED : booking_model_1.BookingStatus.CONFIRMED,
                'payment.status': isFullRefund ? booking_model_1.PaymentStatus.REFUNDED : booking_model_1.PaymentStatus.SUCCESS,
                cancellation_reason: isFullRefund ? `Refunded: ${refundData.reason}` : null,
                cancelled_by: isFullRefund ? 'system' : null,
                payout_status: isFullRefund ? booking_model_1.BookingPayoutStatus.REFUNDED : booking.payout_status, // Set to REFUNDED for full refund, keep existing for partial
            },
        }, { new: true })
            .populate('user', 'id firstName lastName email mobile')
            .populate('center', 'id center_name user email mobile_number')
        ...
        const refundTransaction = new transaction_model_1.TransactionModel({
            booking: booking._id,
            user: booking.user,
        ...
        if (payout) {
            ...
            if (isFullRefund) {
                ...
                else if (payout.status === payout_model_1.PayoutStatus.COMPLETED) {
                    // Payout already completed - mark as refunded (reversal needed)
                    payout.status = payout_model_1.PayoutStatus.REFUNDED;
            ...
            await (0, auditTrail_service_1.createAuditTrail)(auditTrail_model_1.ActionType.PAYOUT_REFUNDED, auditTrail_model_1.ActionScale.CRITICAL, `Payout refunded for booking ${bookingId}: ${refundData.reason}`, 'Payout', payout._id, {
                ...
                logger_1.logger.error('Failed to create audit trail for payout refund', {
                    error,
                    payoutId: payout.id,
                });
            });
        ...
        await (0, auditTrail_service_1.createAuditTrail)(auditTrail_model_1.ActionType.REFUND_INITIATED, auditTrail_model_1.ActionScale.CRITICAL, `Refund initiated for booking ${booking.booking_id || bookingId}: ${refundData.reason}`, 'Booking', booking._id, {
            ...
            logger_1.logger.error('Failed to create audit trail for refund', {
                error,
                bookingId,
            });
        });
        ...
        const centerOwner = center?.user
            ? await user_model_1.UserModel.findById(center.user).select('id email mobile').lean()
            : null;
        // Notify user
        if (user?.email) {
            try {
                (0, notificationQueue_service_1.queueEmail)(user.email, notificationMessages_1.EmailSubjects.BOOKING_REFUNDED_USER, {
                    text: (0, notificationMessages_1.getBookingRefundedUserEmailText)({
                ...
                logger_1.logger.error('Failed to queue email for refund', {
        ...
        if (user?.mobile) {
            try {
                ...
                logger_1.logger.error('Failed to queue SMS for refund', {
                    error: error instanceof Error ? error.message : error,
                    bookingId,
                });
            }
        ...
        // Notify academy
        if (centerOwner) {
            const academyPushNotification = (0, notificationMessages_1.getBookingRefundedAcademyPush)({
            ...
            (0, notification_service_1.createAndSendNotification)({
                ...
                logger_1.logger.error('Failed to send notification for refund', { error, bookingId });
            });
        }
        ...
        logger_1.logger.error('Error creating refund:', {
            error: error.message || error,
            bookingId,
        });
        throw new ApiError_1.ApiError(500, 'Failed to create refund');
    }
};
...
const getRefundDetails = async (refundId) => {
    try {
        const refund = await razorpayRoute_service_1.razorpayRouteService.getRefundDetails(refundId);
        return refund;
    }
    catch (error) {
        if (error instanceof ApiError_1.ApiError) {
        ...
        throw new ApiError_1.ApiError(500, 'Failed to fetch refund details');
    }
};
