...
const blacklistToken = async (token, expiresIn) => {
    try {
        ...
        logger_1.logger.debug('Token blacklisted', { expiresIn });
    }
    catch (error) {
 ...
 */
const blacklistJti = async (jti, expiresIn) => {
    try {
        const redis = getRedisClient();
        ...
        logger_1.logger.debug('JTI blacklisted', { jti, expiresIn: ttl });
    }
    catch (error) {
        logger_1.logger.error('Failed to blacklist JTI:', {
            error: error instanceof Error ? error.message : error,
        });
    }
};
    ...
    try {
        ...
        try {
            ...
            if (decoded) {
                ...
                if (decoded.id) {
                    const userBlacklisted = await (0, exports.isUserBlacklisted)(decoded.id);
                    if (userBlacklisted) {
                        return true;
                    }
                }
...
const blacklistUserTokens = async (userId) => {
    try {
        ...
        logger_1.logger.info('User tokens blacklisted (logout all devices)', { userId });
    }
    catch (error) {
        logger_1.logger.error('Failed to blacklist user tokens:', {
            userId,
            error: error instanceof Error ? error.message : error,
        });
        // Don't throw - blacklisting failure shouldn't break the flow
...
const isUserBlacklisted = async (userId) => {
    try {
        ...
        logger_1.logger.error('Failed to check user blacklist:', {
            userId,
            error: error instanceof Error ? error.message : error,
        });
 ...
 * @param userId - User ID
 */
const clearUserBlacklist = async (userId) => {
    try {
        const redis = getRedisClient();
        const userBlacklistKey = `${USER_BLACKLIST_KEY_PREFIX}${userId}`;
        await redis.del(userBlacklistKey);
        logger_1.logger.info('User blacklist cleared (user logged in)', { userId });
    }
    catch (error) {
        logger_1.logger.error('Failed to clear user blacklist:', {
            userId,
            error: error instanceof Error ? error.message : error,
        });
