    ...
    try {
        ...
        if (redis && distance >= 0) {
            try {
                ...
                logger_1.logger.debug('Distance cached from Haversine calculation', { cacheKey, distance });
            }
            catch (cacheError) {
                logger_1.logger.warn('Failed to cache Haversine distance', { error: cacheError });
            }
        }
        return roundToTwoDecimals(distance);
    }
