...
const getCoachingCenter = async (req, res, next) => {
    try {
        ...
        const { is_deleted, deletedAt, updatedAt, ...filteredCoachingCenter } = coachingCenter;
        const response = new ApiResponse_1.ApiResponse(200, { coachingCenter: filteredCoachingCenter }, (0, i18n_1.t)('coachingCenter.get.success'));
        res.json(response);
    }
    catch (error) {
        next(error);
