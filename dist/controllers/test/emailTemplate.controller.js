...
exports.previewEmailTemplate = exports.getEmailTemplatesList = void 0;
 ...
 */
const getEmailTemplatesList = async (_req, res, next) => {
    try {
        ...
        const response = new ApiResponse_1.ApiResponse(200, { templates }, 'Email templates list retrieved successfully');
        res.json(response);
    }
    catch (error) {
        next(error);
...
exports.getEmailTemplatesList = getEmailTemplatesList;
/**
 * Get sample data for a specific template
 */
const getSampleDataForTemplate = (templateName) => {
...
const previewEmailTemplate = async (req, res, next) => {
    try {
        ...
        if (!templateName) {
            throw new ApiError_1.ApiError(400, 'Template name is required');
        }
        const templateFileName = `${templateName}.html`;
        const template = await (0, email_service_1.loadTemplate)(templateFileName);
        const sampleData = getSampleDataForTemplate(templateName);
        const renderedHtml = await (0, email_service_1.renderTemplate)(template, sampleData);
        ...
        if (error instanceof ApiError_1.ApiError) {
            ...
            logger_1.logger.error('Failed to preview email template:', {
                templateName: req.params.templateName,
                error: error instanceof Error ? error.message : error,
            });
            next(new ApiError_1.ApiError(500, 'Failed to preview email template'));
        }
    }
};
